...
### 修复
- **面板白屏**：`server/static/app.js` 设置页 `onMounted` 块存在字面 `\n` 损坏
  （commit `8d6a816` 引入，v0.6.0 的 AI 配置同步修复）——语法错误导致浏览器解析
  app.js 直接中止、Vue 永不挂载、页面空白。curl 层面（HTTP 200 + 资源可达）无法
  发现。已改为真实换行；`index.html` 缓存版本号提升到 `?v=64`。

### 文档 / 纪律
- 教训记录（决策 #51，本地）：面板验收必须含**渲染级检查**（`node --check app.js`
  + 无头挂载），不能只看 curl 200；镜像 tag `v0.6.2.1` 遵守镜像 tag = git tag 策略。
...
- **6 条新 YAML 规则**：docker_socket_mount、nsenter_escape、privileged_exec、reverse_shell、sensitive_file_access、host_directory_access
