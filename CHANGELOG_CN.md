...
### 修复
- **面板状态配色**：资产表 & 攻击链目标状态 Succeeded=绿 / Failed=红 / Pending=黄；privileged 列标签（否=蓝, 是=红）（v0.6.0 之后）
- **面板人工确认队列**：轮询不再覆盖已展开的容器画像；已判决容器（decisions.log）从待处理队列排除（v0.6.0 之后）
- **面板 AI 配置同步**：保存/激活/删除同步侧边栏快捷面板；aiQuickSave 同步设置页（v0.6.0 之后）
- **bpf_smoke.py**：移除硬编码绝对路径 — 改用 `Path(__file__)` 解析（v0.6.0 之后）
