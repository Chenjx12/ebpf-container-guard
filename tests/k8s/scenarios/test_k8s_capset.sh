...
kubectl exec -n default "$POD" -- /bin/sh -c 'python3 -c "import ctypes; libc=ctypes.CDLL(None); hdr=(ctypes.c_uint32*3)(0x20080522,0,0); data=(ctypes.c_uint32*3)(0x200000,0x200000,0x200000); libc.syscall(126,hdr,data); print(\"CAPSET\")"' 2>/dev/null || { print_fail "exec 触发失败"; cleanup "$POD"; print_summary; }
print_pass
