...
kubectl exec -n default "$POD" -- /bin/sh -c "mkdir -p /tmp/host_proc && mount -t proc proc /tmp/host_proc && echo MOUNTED" 2>/dev/null || { print_fail "exec 触发失败"; cleanup "$POD"; print_summary; }
print_pass
