#!/usr/bin/env python3
"""
Authentication & authorization — authoritative module (v0.6.4).

Shared by the FastAPI panel (server/) and the legacy Streamlit dashboard
(dashboard/auth.py is now a thin re-export shim keeping old import paths
working until the dashboard directory is retired).
...
class AuthManager:
    ...
    def ensure_initial_admin(self) -> str:
        ...
        Called at startup (dashboard or server); password printed to terminal.
        """
        if self.users:
            return ""
